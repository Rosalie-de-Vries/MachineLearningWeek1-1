  ...
  # image 1
  plotRGB(im1_orig,r=4,b=3,g=2,main="Image")                    # we plot bands (4,3,2) (NIR-R-G) as (R,G,B)
  ...
  for(nT in 1:length(numTrees_range)) {
    for(mL in 1:length(minLeaf_range)) {
      ...
      # train Random Forest
      rf_model <- randomForest(y_train[valid_train]~.,data=x_train[valid_train,], ntree=nT, nodesize = mL)
  ...
  rf_model <- randomForest(y_train[valid_train]~.,data=x_train[valid_train,], ntree=nT, nodesize = mL, importance=TRUE)
  ...
  ####### YOUR CODE HERE #######
  rf_model <- randomForest(y_train[valid_train_full]~.,data=vars_keep[valid_train_full,], ntree=numTrees, nodesize = minLeafSize)
  ##############################
